...
//EVERYTHING REQUESTED IN BLACK BELT WORKS, I ALSO INCLUDED EXTRA FEATURES.
//EVERYTHING REQUESTED IN BLACK BELT WORKS, I ALSO INCLUDED EXTRA FEATURES.
//EVERYTHING REQUESTED IN BLACK BELT WORKS, I ALSO INCLUDED EXTRA FEATURES.
//EVERYTHING REQUESTED IN BLACK BELT WORKS, I ALSO INCLUDED EXTRA FEATURES.
@Controller
...
//			List<TaskModel> taskPriority = ;
	...
	public String newIdea(@Valid @ModelAttribute("tasks")TaskModel task, BindingResult result, HttpSession session, RedirectAttributes redirectAttribute) {
		if(result.hasErrors()) {
			redirectAttribute.addFlashAttribute("error", "Please fill out all fields!");
	...
	public String editIdea(@PathVariable("id")Long id,  Model model, HttpSession session, UserModel users, TaskModel tasks) {
		...
		return "redirect:/";
			...
			List<UserModel> user = userService.getAll();
			model.addAttribute("user", user);
			//NAME AND ASSIGNEE WERE CHANGED TO PUBIC IN THE MODELS TO TEST THIS
	//		for(Iterator<String> iter = list.listIterator(); iter.hasNext(); ) {
	//			String a = iter.next();
	//			if(task.getAssignee() != user.name);
	//				iter.remove();
	//		}
			return "edit.jsp";
		}
	...
	public String updateIdea(@PathVariable("id") Long id, 
			@RequestParam(value="task") String taskEdit,
			@RequestParam(value="assignee") String assigneeEdit,
			@RequestParam(value="priority") String priorityEdit){
		
			taskService.update(id, taskEdit, assigneeEdit, priorityEdit);
			return "redirect:/welcome";
	}
